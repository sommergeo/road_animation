...
world <- ggplot() +
  borders(world_shape, colour = 'black', fill = 'black') +
...
## Exponential
map <- world +
  geom_point(data=ROAD_all, aes(x=locality.x, y=locality.y, group=locality.idlocality, col=locality.idlocality),
             size=2, colour = 'lightgoldenrod', alpha = 1) +
  transition_time(time=-ROAD_all$steps_exp) +
  #shadow_mark(past=TRUE, size=2, colour="grey0", alpha=.1) +
  enter_fade() +
  exit_fade() +
  labs(title = '{round(exp(-frame_time))} ka BP') +
  theme(plot.title = element_text(size=18, color='lightgoldenrod'))

# Test animation
animate(map, fps=5, duration=10, start_pause = 0, end_pause = 0, bg='midnightblue')

# Export as gif
anim <- animate(map, fps=30, duration=31, end_pause = 1, renderer = gifski_renderer(loop = TRUE), bg='midnightblue')
anim_save(filename="road_lithics3MaEXP_30sec_30fps_loop_black.gif", animation = anim)
