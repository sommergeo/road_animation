...
ROAD_all <- subset(ROAD_all, query_age_min >= 10)
...
# Test animation
animate(map, fps=5, duration=10, start_pause = 0, end_pause = 0, renderer = gifski_renderer(loop = FALSE))
...
anim_save(filename="road_lithics3Ma-20ka_EXP_30sec_30fps.gif", animation = anim)

anim <- animate(map, fps=30, duration=31, end_pause = 1, renderer = gifski_renderer(loop = TRUE))
anim_save(filename="road_lithics3Ma-20ka_EXP_30sec_30fps_loop.gif", animation = anim)
